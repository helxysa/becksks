...
const contratoOriginalId = ref(null);
...
const fetchContrato = async (id) => {
  try {
    ...
    if (response.data) {
      contratoForm.value = {
        nome_termo: response.data.nomeContrato || '',
        data_inicio: response.data.dataInicio || '',
        data_fim: response.data.dataFim || '',
        saldo_contrato: response.data.saldoContrato || '',
        objeto_contrato: response.data.objetoContrato || '',
        observacoes: response.data.observacoes || '',
        nomeCliente: response.data.nomeCliente || '',
        fiscal: {
          nome: response.data.fiscal.nome || '',
          telefone: response.data.fiscal.telefone || '',
          email: response.data.fiscal.email || '',
        },
        cidade: response.data.cidade || '',
        estado: response.data.estado || '',
        pontoFocal: response.data.pontoFocal || '',
      };
      saldoContratoOriginal.value = response.data.saldoContratoOriginal || 0;
      contratoOriginalId.value = response.data.termoAditivoId;

      if (response.data.porcentagemAjuste) {
        porcentagemAjuste.value = parseFloat(response.data.porcentagemAjuste);
        ajusteValor.value = 'sim';
      } else {
        ajusteValor.value = 'nao';
      }
    }
  } catch (error) {
...
const createTermoAditivo = async (contratoId) => {
  try {
    ...
    let payload = {
      ...
      porcentagem_ajuste: ajusteValor.value === 'sim' ? porcentagemAjuste.value : null,
      // termo_aditivo_itens: termoAditivoItens.value,
    }
    const response = await api.put(`/contratos/${contratoId}`, payload);
